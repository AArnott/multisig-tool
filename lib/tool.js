...
$(function() {
  ...
  $('#step_1 button').on('click', function() {
    ...
    document.vault = vault; // expose to the rest of script on the page.
  ...
  $('#step_2 button').on('click', function() {
    ...
    vault.buildTransaction({
      ...
      onReady: function(err) {
        ...
        //$('.raw code').text(vault.tx.transaction.toHex()); // SKIP: we never built the transaction
        $('.shared_seed code').text(vault.tx.seeds.shared);
